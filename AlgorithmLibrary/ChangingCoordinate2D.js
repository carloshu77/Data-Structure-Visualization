...
ChangeCoordinate2D.prototype.init = function(am, w, h)
{
	...
	this.cmd("CreateLabel", this.robotLabel1ID, "机器空间到世界空间\n(方向)", ChangeCoordinate2D.ROBOT_MATRIX_START_X, ChangeCoordinate2D.ROBOT_MATRIX_START_Y - 25, 0);
	this.cmd("SetForegroundColor", this.robotLabel1ID, "#0000FF");

	this.cmd("CreateLabel", this.robotLabel2ID, "机器空间到世界空间\n(位置)", ChangeCoordinate2D.ROBOT_POSITION_START_X, ChangeCoordinate2D.ROBOT_MATRIX_START_Y - 25, 0);
	...
	this.cmd("CreateLabel", this.handLabel1ID, "手空间到机器空间\n(方向)", ChangeCoordinate2D.HAND_MATRIX_START_X, ChangeCoordinate2D.HAND_MATRIX_START_Y - 25, 0);
	this.cmd("SetForegroundColor", this.handLabel1ID, "#0000FF");

	this.cmd("CreateLabel", this.handLabel2ID, "手空间到机器空间\n(位置)", ChangeCoordinate2D.HAND_POSITION_START_X, ChangeCoordinate2D.HAND_MATRIX_START_Y - 25, 0);
...
ChangeCoordinate2D.prototype.addControls =  function()
{
	...
	var transformButton = addControlToAlgorithmBar("Button", "点变换");
	...
	var radioButtonList = addRadioButtonGroupToAlgorithmBar(["手空间 -> 世界空间", 
															 "世界空间 -> 手空间", 
															 ], 
															"Transform Type");
	...
	var radioButtonList = addRadioButtonGroupToAlgorithmBar(["行优先", 
															 "列优先", 
															 ], 
															"RankType");
	...
	this.showAxisBox = addCheckboxToAlgorithmBar("显示所有轴");
	...
	var moveObjectsButton = addControlToAlgorithmBar("Button", "移动对象");
...
ChangeCoordinate2D.prototype.changeTransformType = function(globalToLocal)
{
	...
	if (globalToLocal)
	{
		this.cmd("SetText", this.robotLabel1ID, "世界空间到机器空间\n(方向)");

	}
	else
	{
		this.cmd("SetText", this.robotLabel1ID, "机器空间到世界空间\n(方向)");
	}
	this.cmd("Step");
	this.RobotMatrix = this.transposeVisual(this.RobotMatrix)
	...
	if (globalToLocal)
	{
		this.cmd("SetText", this.robotLabel2ID, "世界空间到机器空间\n(位置)");
		
	}
	else
	{
		this.cmd("SetText", this.robotLabel2ID, "机器空间到世界空间\n(位置)");
	}
	this.cmd("Step");
	...
	if (globalToLocal)
	{
		this.cmd("SetText", this.handLabel1ID, "机器空间到手空间\n(方向)");
		
	}
	else
	{
		this.cmd("SetText", this.handLabel1ID, "手空间到机器空间\n(方向)");
	}
	...
	if (globalToLocal)
	{
		this.cmd("SetText", this.handLabel2ID, "机器空间到手空间\n(位置)");
		
	}
	else
	{
		this.cmd("SetText", this.handLabel2ID, "手空间到机器空间\n(位置)");
	}
	this.cmd("Step");

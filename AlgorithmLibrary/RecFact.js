...
RecFact.prototype.addControls =  function()
{
	...
	this.factorialButton = addControlToAlgorithmBar("Button", "阶乘");
...
RecFact.prototype.doFactorial = function(value)
{
	...
	this.cmd("CreateLabel", resultID, "阶乘(" + String(value) + ") = " + String(final),  
...
{	
	var activationRec = this.createActivation("阶乘     ", RecFact.ACTIVATION_FIELDS, this.currentX, this.currentY);
...
//	this.cmd("CreateLabel", ID, "", 10, this.currentY, 0);
	...
	if (value > 1)
	{
		...
		this.currentY = oldY;
		this.currentX = oldX;
		this.cmd("CreateLabel", this.nextIndex, "返回值 = " + String(firstValue * value), oldX, oldY);
...
//		this.cmd("Step");
	...
	else
	{
		...
		this.currentX = oldX;
		this.deleteActivation(activationRec);
		this.cmd("CreateLabel", this.nextIndex, "返回值 = 1", oldX, oldY);
		this.cmd("SetForegroundColor", this.nextIndex, Recursive.CODE_HIGHLIGHT_COLOR);
		this.cmd("Step");
		this.cmd("Delete",this.nextIndex);
